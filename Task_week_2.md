...
## 1. Nhiệm vụ của Em (Thành viên A): Tích hợp Pre-commit hooks (Local)

Nhiệm vụ của em là thiết lập một "chốt chặn" ngay trên máy tính của developer. Mỗi khi gõ lệnh `git commit`, hệ thống phải tự động chạy kiểm tra trước khi cho phép commit thành công.
...
## 2. Nhiệm vụ của Bạn Nữ (Thành viên B): Tích hợp SAST & SCA trên CI

Bạn nữ sẽ lo phần "chốt chặn" thứ hai trên GitHub. Khi code vượt qua được máy local và đẩy lên GitHub (Push/Pull Request), CI Pipeline sẽ quét sâu hơn vào mã nguồn và các thư viện phụ thuộc.
...
Khi bắt đầu đưa các công cụ bảo mật vào, các em rất dễ gặp phải 2 vấn đề đau đầu sau:
...
Thầy có thể hướng dẫn chi tiết hơn về cách viết file cấu hình cho công cụ nào trước. Em muốn thầy gợi ý file cấu hình `.pre-commit-config.yaml` cho em, hay đoạn mã YAML tích hợp Semgrep cho bạn nữ?